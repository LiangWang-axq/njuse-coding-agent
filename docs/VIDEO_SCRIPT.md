# 2 分钟演示视频脚本

目标：演示 Coding Agent 用真实模型完成一个编程任务，并简要讲解实现。要求 mp4、≤2 分钟、≤200MB；画面中不得出现 API key 或 .env 内容。

## 分段脚本

### 0:00–0:10 开场

镜头对准终端/项目目录。口播：

> 这是一个不依赖任何 Agent 框架的编程智能体，支持在终端里像聊天一样对话，自主读写文件、执行命令、完成编程任务。下面让它修复一个真实的订单计算 bug。

### 0:10–1:35 真实任务演示

1. 进入 `demo/tasks/fix_me`，展示目录结构；先手动运行 `python -m unittest discover -s tests -v`，展示 2 个失败测试。
2. 运行 `python -m coding_agent` 进入交互模式（设置 `$env:PYTHONPATH = "..\.."`），输入：

   ```powershell
   你 > 检查订单计算模块，修复金额格式化与折扣计算的 bug，运行测试直到全部通过
   ```

3. 镜头跟随流式输出的回答与彩色多行工具卡片（工具名/参数/结果摘要）：`list_files`、`read_file`、`search_code`、`replace_in_file`、`run_tests`；长任务中可顺带展示 `[上下文]` 压缩提示与每轮 token 用量小结。
4. 展示测试由失败变通过，Agent 输出总结与 `（本轮 N 步 · 累计 N 步）`；可再演示 `/status` 与 `/new`。

### 1:35–2:00 设计讲解

口播要点：

- 所有工具在本地执行，路径锁定工作区，拒绝越界与 `.env`；
- 终端对话体验：自研 SSE 流式输出、多轮历史复用、纯标准库 ANSI 界面；工具调用多行卡片、压缩事件实时提示与 token 用量/预算展示；
- 自研四层上下文压缩（大结果落盘、中间裁切、旧结果摘要、超阈才 LLM 摘要）、会话持久化（`--resume` 续跑）、输出解析（JSON / 原生 tool calls / 流式增量）与多工具调用；
- 429/5xx 自动重试，连续解析失败或达到最大步数时安全终止；
- 测试命令白名单，无 shell 拼接。

## 录制与剪辑要点

- OBS 录制：1080p、30fps，仅录制终端窗口；如需要可加一个窄条窗口显示命令。
- 剪映导出：mp4、H.264，码率建议 ≤8Mbps（2 分钟约 120MB，留足余量）。
- 可加字幕，但不要遮挡终端输出；剪掉等待模型的空白时间，可加速但保留关键工具调用帧。
- 提交前检查：时长 ≤2:00、大小 ≤200MB、画面中无 API key / `.env`。
