...
- **L2 旧结果占位**：仍超预算时，把较早的工具结果压成一行摘要（工具名 + 成功/失败 + 关键字段，stdout/stderr 只留长度），最近 6 条消息不动，确保最近 3 组工具交互完整保留，0 API 损耗；
...
- 重复动作终止：相同的只读工具计划累计出现两次后再次出现时，先注入一次纠偏提示并跳过本次调用；模型仍重复时停止本轮。成功执行文件修改后清空该计数并允许重新验证。
